...
def signup():
    ...
    role = data.get("role")
    company_key = data.get("companyKey")
    ...
    if role == "hr" and company_key != "A NEW HR":
        raise ApiError("Invalid Company Key.", 400)
    ...
    db.session.flush()
    notify(user, "user", "Welcome to Axiom! Your account has been created.")
    db.session.commit()
